...
class DataValidation:
    ...
    def is_columns_exist(self,df:DataFrame)->bool:
        ...
        try:
            ...
            if len(missing_categorical_columns)>0:
                logging.info(f"Missing categorical_columns:{missing_categorical_columns}")
    ...
    def initiate_data_validation(self)->DataValidationArtifact:
        ...
        try:
            ...
            logging.info(f"All required columns are present in training data:{status}")
            if not status:
                validation_error_msg+=f"Columns are missing in train data"
            
            status=self.validate_number_of_columns(dataframe=test_df)
            logging.info(f"All required columns are present in testing data:{status}")
            if not status:
                validation_error_msg+=f"Columns are missing in test data"

            status=self.is_columns_exist(df=train_df)
            if not status:
                validation_error_msg+=f"Columns are missing in trainig data"
